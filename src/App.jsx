...
import { properties, priceRanges, marketSignals, trustSignals } from "./data";
import MortgageCalculator from "./components/MortgageCalculator";
import NeighborhoodCompare from "./components/NeighborhoodCompare";
import useLiveMarketData from "./hooks/useLiveMarketData";
import { Search, Map, Grid3X3, Heart, Star, TrendingUp, Landmark, Sparkles, X } from "lucide-react";
...
export default function App() {
  ...
  const filteredProperties = useMemo(() => {
    ...
    return list;
  }, [filters, listingTab, page, savedIds, searchQuery]);
  ...
  return (
    <div className="min-h-screen bg-dark-900">
        ...
        <>
          ...
          <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10 relative z-10">
            ...
            <p className="text-white/30 text-xs mt-3 text-right">
              Live public FRED data {lastUpdated ? `updated ${lastUpdated}` : "loading..."}
            </p>
          </section>
          ...
          <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
            <div className="flex items-center justify-between mb-10">
              <div>
                ...
                <h2 className="font-display text-3xl sm:text-4xl font-bold text-white">
                  Handpicked homes across the United States
                </h2>
                <p className="text-white/45 mt-2 max-w-2xl">
                  Browse the most active listings in premium U.S. metros, all priced in dollars and ready to compare.
                </p>
              </div>
              <button onClick={() => setPage("listings")} className="btn-secondary text-sm hidden sm:flex">
                View All →
              </button>
            </div>
            ...
            <div className="text-center mt-10">
              <button onClick={() => setPage("listings")} className="btn-primary">
                Explore U.S. listings
              </button>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-6">
              <div className="bg-dark-800/60 border border-white/10 rounded-3xl p-6 sm:p-8">
                ...
                <h2 className="font-display text-2xl sm:text-3xl font-bold text-white mb-6">
                  Follow price and rent momentum by metro
                </h2>
              ...
              <div className="bg-dark-800/60 border border-white/10 rounded-3xl p-6 sm:p-8">
                ...
                <h2 className="font-display text-2xl sm:text-3xl font-bold text-white mb-6">
                  High-intent metros in the U.S. market
                </h2>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center max-w-3xl mx-auto mb-12">
                ...
                <h2 className="font-display text-3xl font-bold text-white mb-3">
                  Built for serious U.S. home search
                </h2>
                <p className="text-white/45">
                  This experience is centered on current U.S. inventory, price comparisons, and the kind of filters buyers expect from a Zillow-style product.
                </p>
              </div>
          ...
          <footer className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center border-t border-white/5 mt-0">
            ...
            <p className="text-white/30 text-sm">© 2025 NestFinder Inc. U.S. market data shown in USD.</p>
            <div className="flex justify-center gap-6 mt-4">
              {["About", "Agents", "Blog", "Privacy", "Terms"].map((label) => (
                <button key={label} className="text-white/30 hover:text-white/60 text-xs transition-colors">
                  {label}
                </button>
              ))}
            </div>
        ...
        <div className="pt-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div className="mb-8">
              ...
              {page !== "saved" && (
                <p className="text-white/45 mt-2 max-w-2xl">
                  Filter the U.S. market by city, price, and property type. All values are displayed in dollars.
                </p>
              )}
            </div>
                ...
                <div className="relative z-10 p-5 sm:p-6 lg:p-8 space-y-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="w-full lg:w-auto lg:min-w-[400px]">
                      <label className="text-xs text-white/65 font-medium uppercase tracking-wider block mb-2">
                        Search homes and cities
                      </label>
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                      <label
                        htmlFor="listings-bg-upload"
                        className="px-4 py-2 rounded-xl text-xs font-medium uppercase tracking-wide cursor-pointer border border-white/20 bg-black/30 text-white/85 hover:bg-black/45 transition-colors"
                      >
                        Upload Background Image
                      </label>
                      <input
                        id="listings-bg-upload"
                        type="file"
                        accept="image/*"
                        onChange={handleListingsBgUpload}
                        className="hidden"
                      />
                      ...
                      <div className="flex bg-black/35 border border-white/15 rounded-xl p-1 gap-1 backdrop-blur-sm">
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm transition-all ${
                            viewMode === "grid" ? "bg-brand-500 text-white" : "text-white/70 hover:text-white"
                          }`}
                        >
                          <Grid3X3 size={14} />
                          Grid
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm transition-all ${
                            viewMode === "map" ? "bg-brand-500 text-white" : "text-white/70 hover:text-white"
                          }`}
                        >
                          <Map size={14} />
                          Map
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div className="bg-black/35 border border-white/15 rounded-2xl p-4 backdrop-blur-sm">
                      ...
                      <p className="text-2xl font-display font-bold text-white">
                        {listingSummary.averagePrice ? formatMoney(Math.round(listingSummary.averagePrice)) : "$0"}
                      </p>
                    </div>
                    <div className="bg-black/35 border border-white/15 rounded-2xl p-4 backdrop-blur-sm">
                      <p className="text-white/55 text-xs uppercase tracking-[0.2em] mb-1">Sale / rent mix</p>
                      <p className="text-2xl font-display font-bold text-white">
                        {listingSummary.saleCount} / {listingSummary.rentCount}
                      </p>
                    </div>
                  </div>
            ...
            {viewMode === "map" && page !== "saved" && (
              <div className="mb-8">
                <MapView properties={filteredProperties} onSelect={handleSelectProperty} />
              </div>
            )}
                ...
                {filteredProperties.length === 0 ? (
                  <div className="text-center py-20">
                    ...
                    <h3 className="font-display text-2xl text-white mb-2">
                      {page === "saved" ? "No Saved Properties" : "No Results Found"}
                    </h3>
      ...
      {page === "detail" && selectedProperty && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_0.9fr] gap-8 mt-2">
            <MortgageCalculator price={selectedProperty.price} />
            <div className="bg-dark-800 border border-white/10 rounded-3xl p-6 space-y-5">
              <div className="flex items-center gap-2 text-brand-500 text-sm font-medium">
                <Landmark size={14} />
                Local market snapshot
              </div>
              <h3 className="font-display text-2xl font-bold text-white">Why this listing stands out in its metro</h3>
              <p className="text-white/45 leading-relaxed">
                {selectedProperty.city} is one of our featured U.S. markets, with demand shaped by location, job centers, and pricing relative to the local median.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="rounded-2xl border border-white/5 bg-dark-700/60 p-4">
                  <p className="text-white/35 text-xs uppercase tracking-[0.18em] mb-2">Asking price</p>
                  <p className="text-lg font-semibold text-white">{formatMoney(selectedProperty.price)}</p>
                </div>
                <div className="rounded-2xl border border-white/5 bg-dark-700/60 p-4">
                  <p className="text-white/35 text-xs uppercase tracking-[0.18em] mb-2">Market type</p>
                  <p className="text-lg font-semibold text-white capitalize">{selectedProperty.listing === "sale" ? "For sale" : "For rent"}</p>
                </div>
                <div className="rounded-2xl border border-white/5 bg-dark-700/60 p-4">
                  <p className="text-white/35 text-xs uppercase tracking-[0.18em] mb-2">Bedrooms</p>
                  <p className="text-lg font-semibold text-white">{selectedProperty.bedrooms}</p>
                </div>
                <div className="rounded-2xl border border-white/5 bg-dark-700/60 p-4">
                  <p className="text-white/35 text-xs uppercase tracking-[0.18em] mb-2">Bathrooms</p>
